...
subject to Donate_Receive {i in DONORS}: sum{j in RECIPIENTS} X[i,j]*compat_pairs[i,j] = sum{j in DONORS} X[j,i]*compat_pairs[j,i];
...
subject to Kidney_Donate {i in DONORS}: sum{j in RECIPIENTS} X[i,j]*compat_pairs[i,j] <= 1; 
...
subject to Cycle_Constraint {(i,j,k,l) in PAIRS}: compat_pairs[i,j]*X[i,j] + compat_pairs[j,k]*X[j,k] + compat_pairs[k,l]*X[k,l] <= 2;
>>>>>>> 788349eea30fd4ac5f08bc6a530867c598b5cebc
